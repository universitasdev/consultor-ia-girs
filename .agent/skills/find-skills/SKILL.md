...
### Windows Troubleshooting (WSL and Git Bash)

If you are running this script in a Windows environment, you might encounter two common issues:

1. **WSL Error (`execvpe(/bin/bash) failed`)**:
   This occurs if your default or only WSL distribution is `docker-desktop` (which does not provide a full Linux environment or bash). To work around this:
   - Bypass WSL and use **Git Bash** instead.
   - Run the script by explicitly invoking the Git Bash executable from PowerShell:
     ```powershell
     & "C:\Program Files\Git\bin\bash.exe" scripts/find.sh "<your search query>"
     ```

2. **Missing Dependency Error (`jq is required`)**:
   The script strictly depends on `jq` to parse JSON API results. On Windows, you can cleanly install it using `winget`:
   ```powershell
   winget install jqlang.jq --accept-source-agreements --accept-package-agreements
   ```
   *Note:* After installation, if the current shell session does not recognize the command, manually inject the installation path or restart the terminal before launching Git Bash:
   ```powershell
   $env:PATH = "C:\Users\$env:USERNAME\AppData\Local\Microsoft\WinGet\Packages\jqlang.jq_Microsoft.Winget.Source_8wekyb3d8bbwe;" + $env:PATH
   & "C:\Program Files\Git\bin\bash.exe" scripts/find.sh "<your search query>"
   ```
